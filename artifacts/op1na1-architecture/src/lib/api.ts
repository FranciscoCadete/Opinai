...
export async function trackRequest(ticketId: string): Promise<TrackResponse> {
  const res = await fetch(
    `${API_BASE}/requests/${encodeURIComponent(ticketId)}`,
  );
...
export async function updateAdminRequest(
  ...
  const res = await fetch(
    `${API_BASE}/admin/requests/${encodeURIComponent(id)}`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
...
export async function listAdminAuditLog(
  ...
  const qs = params.toString();
  const res = await fetch(
    `${API_BASE}/admin/audit-log${qs ? `?${qs}` : ""}`,
    { credentials: "include" },
  );
