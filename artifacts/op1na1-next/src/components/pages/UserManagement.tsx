...
export default function UserManagement() {
  ...
  async function saveForm() {
    ...
    try {
      if (editId === null) {
        const created = await createAdminUser({ email: form.email, name: form.name, password: "Password123!", role });
        const newUser: User = { ...form, id: created.id, createdAt: new Date(created.createdAt).toLocaleDateString("pt-AO"), lastActivity: "—" };
        setUsers(prev => [newUser, ...prev]);
        addAudit("Criou utilizador", form.name, `Perfil: ${form.profile}`, "create");
      } else {
        await updateAdminUser(String(editId), { name: form.name, role });
  ...
  async function executeDelete() {
    ...
    try {
      if (typeof deleteTarget === "string") await deleteAdminUser(deleteTarget);
    ...
    <main id="main-content" className="flex flex-col gap-4 min-h-0">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-foreground">Gestão de Utilizadores</h1>
      ...
      <div className="flex flex-wrap items-center gap-2 bg-card dark:bg-zinc-900 border border-border dark:border-zinc-800 rounded-xl px-4 py-3">
        ...
        {([
          ...
          <select key={f.label} value={f.value} aria-label={`Filtrar por ${f.label}`}
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 bg-primary/5 dark:bg-primary/10 border border-primary/20 rounded-xl px-4 py-2.5">
          ...
          <button onClick={bulkSuspend} className="flex items-center gap-1 text-xs font-medium text-red-600 dark:text-red-400 hover:underline">
            <Ban aria-hidden="true" size={15} /> Suspender
          </button>
          <button onClick={bulkActivate} className="flex items-center gap-1 text-xs font-medium text-green-600 dark:text-green-400 hover:underline">
              ...
              <div className="absolute top-6 left-0 z-50 bg-card dark:bg-zinc-800 border border-border rounded-xl shadow-xl py-1 min-w-32">
                {PROFILES.map(p => (
                  <button key={p} onClick={() => bulkChangeProfile(p)}
                    className="block w-full text-left px-3 py-1.5 text-xs hover:bg-secondary font-medium">{p}</button>
                ))}
              </div>
            )}
          </div>
          <button onClick={() => setSelected(new Set())} className="ml-auto text-xs text-muted-foreground hover:text-foreground">Cancelar</button>
        </div>
      )}
      ...
      <div className="bg-card dark:bg-zinc-900 border border-border dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm">
              ...
              <tr className="border-b border-border bg-secondary/50 dark:bg-zinc-800/60">
                <th className="px-3 py-3 w-10">
                  <input type="checkbox" checked={allOnPageSelected} onChange={toggleAll}
                    aria-label="Seleccionar todos" className="rounded border-border cursor-pointer accent-primary" />
                </th>
                <th className="px-3 py-3 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">Utilizador</th>
                <th className="px-3 py-3 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">{t("users.columns.role", "Perfil")}</th>
                ...
                <th className="px-3 py-3 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">{t("users.columns.createdAt", "Registo")}</th>
                ...
                <th className="px-3 py-3 text-center font-semibold text-muted-foreground text-xs uppercase tracking-wider">{t("users.columns.actions", "Acções")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border dark:divide-zinc-800">
              {paginated.length === 0 && (
                <tr><td colSpan={8} className="text-center py-12 text-muted-foreground text-sm">Nenhum utilizador encontrado.</td></tr>
              )}
              {paginated.map(u => (
                <tr key={u.id} className={cn("hover:bg-secondary/30 dark:hover:bg-zinc-800/40 transition-colors", selected.has(u.id) && "bg-primary/5 dark:bg-primary/10")}>
                  <td className="px-3 py-3 w-10">
                    <input type="checkbox" checked={selected.has(u.id)} onChange={() => toggleSelect(u.id)}
                      aria-label={`Seleccionar ${u.name}`} className="rounded border-border cursor-pointer accent-primary" />
                  </td>
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0",
                        u.profile === "Admin" ? "bg-red-500" : u.profile === "Gestor" ? "bg-purple-500" :
                        u.profile === "Analista" ? "bg-blue-500" : u.profile === "Técnico" ? "bg-green-600" : "bg-zinc-500"
                      )}>{initials(u.name)}</div>
                      <div>
                  ...
                  <td className="px-3 py-3">
                    <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-full", PROFILE_STYLE[u.profile])}>{u.profile}</span>
                  </td>
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-1 max-w-44">
                      {u.neighborhoods.slice(0, 2).map(b => (
                        <span key={b} className="text-[10px] bg-secondary dark:bg-zinc-700 text-muted-foreground px-1.5 py-0.5 rounded font-medium">{b}</span>
                      ))}
                      {u.neighborhoods.length > 2 && (
                        <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold">+{u.neighborhoods.length - 2}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-3 py-3">
                    <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-full", STATUS_STYLE[u.status])}>{u.status}</span>
                  </td>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center justify-center gap-1">
                      <button aria-label="Editar" onClick={() => openEdit(u)} title="Editar"
                      ...
                      <button aria-label="Eliminar" onClick={() => confirmDelete(u.id)} title="Eliminar"
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border dark:border-zinc-800 bg-secondary/30">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} aria-label="Próxima página"
              className="p-1.5 rounded-lg border border-border hover:bg-secondary disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="modal-title"
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border dark:border-zinc-700 sticky top-0 bg-card dark:bg-zinc-900 z-10">
              ...
              <button onClick={() => setShowForm(false)} aria-label="Fechar" className="p-2 rounded-lg hover:bg-secondary transition-colors">
                <X aria-hidden="true" size={15} />
              </button>
            </div>
            <div className="px-6 py-5 space-y-4">
              {formError && (
                <div role="alert" className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm rounded-lg px-3 py-2">
                  {formError}
                </div>
              )}
              <div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    Bairros a Gerir * <span className="normal-case font-normal">({form.neighborhoods.length}/{BAIRROS.length})</span>
                  </label>
                ...
                <div className="grid grid-cols-2 gap-1.5 bg-secondary/40 dark:bg-zinc-800/40 rounded-xl p-3 border border-border dark:border-zinc-700">
                  {BAIRROS.map(b => (
                    <label key={b} className={cn("flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-colors text-sm",
                      form.neighborhoods.includes(b) ? "bg-primary/10 text-primary font-medium" : "hover:bg-secondary text-foreground")}>
          ...
          <div className="relative bg-card dark:bg-zinc-900 border border-border rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setDeleteTarget(null)}
                className="px-4 py-2 rounded-lg border border-border text-sm font-medium hover:bg-secondary transition-colors">Cancelar</button>
              <button onClick={executeDelete}
                className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors">Eliminar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-card dark:bg-zinc-900 border-l border-border dark:border-zinc-700 shadow-2xl flex flex-col h-full">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border dark:border-zinc-700">
              ...
              <button onClick={() => setShowAudit(false)} className="p-1.5 rounded-lg hover:bg-secondary transition-colors">
                <X aria-hidden="true" size={15} />
              </button>
            </div>
              ...
              {filteredAudit.map((e, i) => (
                <div key={e.id} className="flex gap-3 py-3 border-b border-border/50 dark:border-zinc-800/50 last:border-0">
                  <div className="flex flex-col items-center">
                    ...
                    {i < filteredAudit.length - 1 && <span className="w-px flex-1 bg-border dark:bg-zinc-700 mt-1 min-h-4" />}
                  </div>
            ...
            <div className="px-5 py-3 border-t border-border dark:border-zinc-700">
              <p className="text-xs text-muted-foreground text-center">Retenção: 12 meses · Exportação via CSV disponível</p>
            </div>
          </div>
        </div>
      )}
